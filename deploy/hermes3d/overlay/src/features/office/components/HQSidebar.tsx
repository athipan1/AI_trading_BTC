    ...
    >
      ...
      >
        ...
        <button
          ...
          className={`min-h-12 flex-1 px-3 py-2 font-mono text-[10px] font-semibold tracking-[0.12em] transition-colors md:flex-none md:rounded-l-md md:border md:border-r-0 md:px-1.5 md:py-2.5 md:tracking-[0.2em] md:shadow-xl md:backdrop-blur ${
            analyticsOnly
              ? "bg-amber-500/15 text-amber-200 md:border-amber-400/50 md:bg-[#1a1206]/95"
              : "text-amber-300/85 active:bg-amber-500/10 md:border-amber-500/25 md:bg-[#120d06]/90 md:hover:border-amber-400/45 md:hover:text-amber-100"
          }`}
      ...
      {open ? (
        <div
          data-hq-panel
          className={`pointer-events-auto mx-2 mb-2 flex max-h-[68dvh] flex-col overflow-hidden rounded-lg border border-cyan-500/20 bg-black/90 shadow-2xl backdrop-blur md:mx-0 md:mb-0 md:h-full md:max-h-none md:rounded-none md:border-y-0 md:border-r-0 md:border-l ${
            boardLikeWidth ? "w-auto md:w-[min(94vw,1180px)]" : "w-auto md:w-56"
          }`}
        >
          <div className="border-b border-cyan-500/15 px-3 py-2.5 md:px-4 md:py-3">
            <div className="font-mono text-[10px] font-semibold tracking-[0.24em] text-cyan-300/80 md:tracking-[0.32em]">
              {analyticsOnly ? "ANALYTICS" : "HEADQUARTERS"}
            </div>
            <div className="mt-1 hidden font-mono text-[11px] text-white/45 sm:block">
              {analyticsOnly
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {!railOnly && onAddAgent ? (
                <button
                  type="button"
                  onClick={onAddAgent}
                  className="min-h-9 rounded border border-cyan-500/20 bg-cyan-500/10 px-2 py-1 font-mono text-[10px] uppercase tracking-[0.12em] text-cyan-200"
                >
                  Add Agent
                </button>
              ) : null}
              {!railOnly && onOpenCompanyBuilder ? (
                <button
                  ...
                  className="min-h-9 rounded border border-emerald-500/20 bg-emerald-500/10 px-2 py-1 font-mono text-[10px] uppercase tracking-[0.12em] text-emerald-200"
                >
                  Build Company
                </button>
              ) : null}
          ...
          {!railOnly ? (
            <div
              ...
              className="grid grid-cols-4 overflow-x-auto border-b border-cyan-500/15"
            >
              {PRIMARY_TABS.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`min-h-11 min-w-0 border-r border-cyan-500/10 px-1 py-2 font-mono text-[9px] uppercase tracking-[0.08em] transition-colors last:border-r-0 md:px-2 md:py-2.5 md:text-[11px] md:tracking-[0.18em] ${
                      isActive
                  ...
                  >
                    ...
                    {showBadge ? (
                      <span className="ml-1 rounded bg-cyan-500/15 px-1 py-0.5 text-[9px] text-cyan-300" aria-label={`${inboxCount} unread`}>
                        {inboxCount}
                      </span>
                    ) : null}
                  </button>
