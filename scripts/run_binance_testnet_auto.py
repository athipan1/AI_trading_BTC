...
def main() -> None:
    ...
    primary = traders[0]
    trade_path = TradePathObserver(primary.position_store)
    ...
    while True:
        ...
        try:
            live_price = primary.broker.current_price(primary.symbol)
            observed = trade_path.observe_open_positions(primary.symbol, float(live_price))
            print(
                json.dumps(
                    {
                        "event": "TRADE_PATH_OBSERVATION",
                        "source": "spot_live_price",
                        "price": live_price,
