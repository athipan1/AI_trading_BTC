...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run automated Phase 5.6.2 frozen forward OOS evidence accumulation"
    )
    ...
    parser.add_argument(
        "--until",
        default=None,
        help="Optional UTC exclusive end. Defaults to the latest fully closed H1 boundary.",
    )
...
def main() -> None:
    ...
    automation = ForwardOOSAutomation(
        config=ForwardOOSAutomationConfig(
    ...
    requested_until = args.until or "auto"
    dataset_run_id = (
        f"phase562-{symbol.replace('/', '')}-{timeframe}-{args.boundary}-{requested_until}"
    ...
    if args.output:
        ...
        print(f"Phase 5.6.2 report: {output}")
